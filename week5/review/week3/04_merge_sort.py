...
def merge(arr, left, mid, right):
    ...
    while idx_left < len(temp_left) and idx_right < len(temp_right):
        if temp_left[idx_left] <= temp_right[idx_right]:
            arr[idx_current] = temp_left[idx_left]
            idx_current += 1
            idx_left += 1
        else:
            arr[idx_current] = temp_right[idx_right]
            idx_current += 1
            idx_right += 1
    ...
    while idx_left <= (len(temp_left)-1):
        arr[idx_current] = temp_left[idx_left]
        idx_left += 1
        idx_current += 1

    while idx_right <= (len(temp_right)-1):
        arr[idx_current] = temp_right[idx_right]
        idx_right += 1
        idx_current += 1
